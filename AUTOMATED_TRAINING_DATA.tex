...
\figure\begin{tikzpicture}[
  ...
  \node[block] (pretrainedmodel) {Pre-trained Text-to-Image Model};
  \node[block, below of=pretrainedmodel] (formisolateddataset) {Form-Isolated Dataset};
  \node[block, below of=formisolateddataset] (lorafinetuning) {LoRA Fine-tuning};
  \node[block, below of=lorafinetuning] (trainedmodel) {Trained AI Model (Form-Aware)};

  \draw[arrow] (pretrainedmodel) -- (lorafinetuning);
  \draw[arrow] (formisolateddataset) -- (lorafinetuning);
  \draw[arrow] (lorafinetuning) -- (trainedmodel);
